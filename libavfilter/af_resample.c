...
static void convert_channel_layout(AVFilterLink *link)
{
    ...
    AVFilterSamplesRef *insamples = resample->s16_samples_ptr;
    ...
    if (resample->reconfig_channel_layout || !resample->channel_conversion) {
        ...
        int out_sample_size = av_get_bits_per_sample_format(insamples->sample_fmt) >> 3;

        int size = num_channels*out_sample_size*insamples->samples_nb;

        if (outsamples)
            avfilter_unref_samples(outsamples);
        outsamples = avfilter_get_samples_ref(link, AV_PERM_WRITE, size,
                                              out_channel, insamples->sample_fmt, 0);
        /*
...
{
    ResampleContext *resample = link->dst->priv;
    AVFilterSamplesRef *insamples = resample->temp_samples_ptr;
